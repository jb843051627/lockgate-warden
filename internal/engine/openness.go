...
//  1. 任一维度 critical（critical 水位差 / 闸门错位 / 结构 red）→ closed；
//  2. 检修 active 锁存在 → maintenance（同闸室至多一把，互斥语义由 service 保证）;
//  3. 任一维度 restricted → restricted；
//  4. 其余 → open。
func Synthesize(dims []Dimension, holdActive bool) model.LockStatus {
	for _, d := range dims {
		if d.Level == LevelCritical {
			return model.LockClosed
		}
	}
	if holdActive {
		return model.LockMaintenance
	}
	for _, d := range dims {
		if d.Level == LevelRestricted {
			return model.LockRestricted
		}
	}
	return model.LockOpen
}

// Explain 输出合成过程的人类可读说明，写入评估备注。
func Explain(dims []Dimension, holdActive bool, level model.LockStatus) string {
