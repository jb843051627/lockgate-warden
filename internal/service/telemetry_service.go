...
func cacheSnapshot(hb model.SensorHeartbeat, batchID int64) cache.SensorSnapshot {
	return cache.SensorSnapshot{
		...
		Kind:     string(hb.Kind),
		Value:    hb.Value,
		Quality:  string(hb.Quality),
		BatchID:  batchID,
	}
}

// IngestBatch 批量遥测入库链，顺序即设计文档约定：
//  1. 结构校验 → 2. 闸室存在 → 3. 校验和重算比对 → 4. 时间窗
...
func (s *Service) IngestBatch(in model.BatchInput) (model.BatchResult, error) {
	...
	chamber, err := s.store.GetChamberByCode(in.ChamberCode)
	if err != nil {
		s.reject()
		return result, fmt.Errorf("chamber %s: %w", in.ChamberCode, err)
	}
	...
	kept, _ := validation.FilterBatchPoints(in.Points, in.WindowStart, in.WindowEnd)
	kept = in.Points
	if len(kept) == 0 {
		s.reject()
	...
	for _, p := range kept {
		...
		if err := validation.CheckSensorOwnership(sensor, chamber.ID); err != nil {
			s.reject()
			return result, err
		}
	...
	if batch.ID <= 0 {
		return result, fmt.Errorf("batch insert returned invalid id")
	}
	rows := make([]store.StoredPointRow, 0, len(kept))
	for _, p := range kept {
		entry := sensors[p.SensorCode]
		quality := validation.Grade(entry.spec, p.Value)
		tally.Add(quality)
		rows = append(rows, store.StoredPointRow{
	...
	inserted, err := s.store.InsertPointsORIgnore(batch.ID, rows)
	if err != nil {
		return result, err
	}
	...
	for idx, p := range kept {
		...
		if s.pipeline != nil {
			ingest.EnqueueSnapshotRefresh(s.pipeline, s.store, s.cache, model.SensorHeartbeat{
				SensorID: entry.sensor.ID,
				Code:     entry.sensor.Code,
				Kind:     entry.sensor.Kind,
			}, batch.ID)
		}
	}

	alerts, err := s.evaluateThresholds(chamber.ID, now)
	if err == nil {
...
func (s *Service) touchHeartbeat(sen model.GateSensor, value float64, quality model.Quality, at time.Time, batchID int64) {
	_ = s.store.TouchHeartbeat(sen.ID, value, quality, at, batchID)
}
