...
func (s *Store) InsertBatch(b *model.TelemetryBatch) error {
	...
	if n == 0 {
		existing := s.db.QueryRow(`SELECT id FROM telemetry_batches WHERE chamber_id=? AND checksum=?`, b.ChamberID, b.Checksum)
		var id int64
...
func (s *Store) InsertPointsORIgnore(batchID int64, points []StoredPointRow) (int64, error) {
	...
	err := s.Transaction(func(tx *sql.Tx) error {
		...
		for _, p := range points {
			if _, err := stmt.Exec(batchID, p.SensorID, p.Seq, formatTime(p.TakenAt), p.Value, string(p.Quality), now); err != nil {
				return err
			}
			inserted++
		}
